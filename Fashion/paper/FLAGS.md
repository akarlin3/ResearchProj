...
## OUT OF SCOPE — remains OPEN (highest-severity assessor item)

- **R2 significance axis.** The highest-severity reviewer concern — that the contribution's
  significance needs **new in-vivo validation data** (cohort-level, with ground truth) — is
  **out of scope for this prose-and-formatting run and remains open.** Checkpoint 4 reframes
  the contribution as a transferable diagnostic (information-floor auditing) to *mitigate* the
  "one-NPE cautionary tale" framing, but it does **not close** this axis: closing it requires
  acquiring and analyzing new in-vivo data, which is a research task, not an editing task.
