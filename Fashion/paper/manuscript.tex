...
does not transfer across acquisitions. On an open in-vivo brain dataset ($N = 500$ voxels),
held-out-b posterior-predictive coverage collapsed to 0.03 at nominal 0.95 (NLLS 0.90).
...
A single in-vivo abdominal case is shown in Supplementary Figure S4 as a qualitative
illustration of the same weak-identifiability signature (the NLLS \Dstar{} reference
...
The in vivo signal evaluation relies on a single open brain dataset ($N = 500$ voxels)
...
while NLLS stays calibrated. (B) The same check on an open multi-b dataset ($N=500$ voxels):
