...
\Dstar{} must be accompanied by an honest uncertainty estimate to be clinically usable ---
yet the calibration of IVIM uncertainty has not been systematically established across
estimation paradigms or the increasingly common deep-learning methods; recent work
quantifying deep-learning IVIM uncertainty (18,19) does not address the cross-paradigm
...
unifying finding is methodological: reliable IVIM uncertainty in the clinical SNR regime
benefits heavily from shape-aware intervals and information-floor benchmarking, because the
standard aggregate checks pass even when the uncertainty is not trustworthy.
...
generally exceeds the Gaussian CRLB, so the true floor lies above the value we plot, and the
