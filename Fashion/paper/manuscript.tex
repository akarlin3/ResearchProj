...
\textbf{Multi-subject cohort confirmation.} To test whether this in-vivo overconfidence is
idiosyncratic to one subject, we repeated the held-out-b check across an open multi-subject
liver cohort ($N = 59$ patients; an independent vendor and field strength --- GE 1.5T --- and a
different organ and b-scheme from the trained acquisition) [23]. This is therefore an
off-scheme deployment for the amortized NPE, the multi-subject analogue of the acquisition-shift
result above. Per patient we scored held-out-b posterior-predictive coverage for the NPE and for
the per-voxel NLLS reference (Supplementary Figure S6). The amortized NPE under-covered held-out
signal in \emph{every} patient relative to NLLS (59/59): cohort-mean coverage at nominal 0.95 was
0.58 (NPE) versus 0.78 (NLLS), with the NPE below NLLS at every credibility level. As in the
single-subject check, voxel-level ground truth is unavailable, so this compares the two
estimators rather than either against truth; NLLS is itself below nominal in vivo (model misfit
and inter-series noise), but the systematic NPE-below-NLLS gap across an independent cohort shows
the amortized overconfidence is a reproducible property under acquisition shift, not a
...
\bibitem{liver2020} \DJ oki\'c-Kova\v{c} J, Dakovi\'c M, Jankovi\'c A, Mitrovi\'c M, Dugali\'c V,
Galun D, \DJ uri\'c-Stefanovi\'c A, Ma\v{s}ulovi\'c D. The role of intravoxel incoherent motion
(IVIM) parameters in characterization of hypovascular liver lesions: the raw diffusion-weighted
images and IVIM data. Dryad; 2020. doi:10.5061/dryad.xwdbrv1cg.
