...
% impossibility (Theorem~\ref{thm:detect}(i)) is a drafted data-processing argument flagged for human
% proof-review (theory/impossibility.md) -- it is stated as such, not as a machine-verified result.
...
\emph{(i) Hidden-channel impossibility} (\emph{drafted; requires human proof-review}). For the hidden
channel ($\delta_{\mathrm{obs}}=0$), every label-free statistic $M=f(O)$ has identical law under
\emph{fresh} ($\delta_{\mathrm{hid}}=0$) and \emph{stale} ($\delta_{\mathrm{hid}}>0$) deployment;
hence every detector built from $O$ has true-positive rate equal to its false-positive rate at every
operating point, its ROC is the diagonal, and its detection AUC is exactly $\tfrac12$.
\end{theorem}
...
\paragraph{Argument for (i) (data-processing / sufficiency; \texttt{theory/impossibility.md}).}
The hidden shift perturbs only $\theta$, so the observable law is exactly invariant,
...
is $\equiv 1$, so no test has power and $\mathrm{AUC}=\tfrac12$. The simulation realises the structural
premise bit-for-bit: the reported points are identical under $\delta_{\mathrm{hid}}=0$ versus $0.20$,
and the monitor returns $M=0.007248$ in both cases.
% src: theory/detectability.py, seed 4242, 200k voxels: M(delta_hid=0)=M(delta_hid=0.20)=0.007248, points identical.
\textbf{Part~(i) is a sufficiency proof, not algebra, and carries a human-review flag} (see
\texttt{theory/impossibility.md}); it is consistent with --- not verified by --- the simulated
hidden-channel AUC $=0.500$. The population statement concerns the idealised law; no finite-sample
concentration claim is made.
...
moves $P_O$; part~(b) inherits the human-review flag of Theorem~\ref{thm:detect}(i), since it rests on
the same exact-invariance premise (the \emph{definition} of ``hidden'' in this model).
